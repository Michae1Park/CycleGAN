...
def generator_resnet(image, options, reuse=False, name="generator"):
    ...
    with tf.variable_scope(name):
        ...
        c1 = tf.nn.relu(instance_norm(conv2d(c0, options.gf_dim, ks, 1, padding='VALID', name='g_e1_c'), 'g_e1_bn'))
        ...
        d2 = tf.pad(d2, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
        pred = tf.nn.tanh(conv2d(d2, options.output_c_dim, ks, 1, padding='VALID', name='g_pred_c'))
...
def huber_criterion(in_, target):
    return tf.reduce_mean(tf.losses.huber_loss(predictions=in_, labels=target))

def mse_criterion(in_, target):
    return tf.reduce_mean(tf.losses.mean_squared_error(predictions=in_, labels=target))
